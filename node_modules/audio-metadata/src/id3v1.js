...
module.exports = function(buffer) {
	...
	try {
        var titleSize = 30; //older: 30
        var artistSize = 30; //older: 30
        var albumSize = 30; //older: 30
        var yearSize = 4; //older: 4
		var offset = view.byteLength - 128 + 3,
			readAscii = utils.readAscii;
		var title = readAscii(view, offset, titleSize),
			artist = readAscii(view, offset + titleSize, artistSize),
			album = readAscii(view, offset + (titleSize + artistSize), albumSize),
			year = readAscii(view, offset + (titleSize + artistSize + albumSize), yearSize);

		offset += (titleSize + artistSize + albumSize + yearSize);
